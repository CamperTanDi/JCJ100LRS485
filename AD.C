...
//	int exp;
	...
	if(SHT10_ReadValue(adCHS,(unsigned int *)&adint[adCHS]))goto loopreset;	
	adCHS++;
	if(adCHS>=2)
	{
		adCHS=0;				
...
//		adfloat[2] = (adfloat[2] )*237.3/(7.5-adfloat[2]);	
		...
		else
		{
			if(HotFlag&0x04)
			{	
				if(adInfoCopy.pv[0]>adInfo.pv[0]+30  || adhotcount>2000)
				{		
